...
class _RunnerDatabricksAuth(httpx.Auth):
    ...
    def __init__(
        ...
        :param server_url: Omnigent server URL used to look up the SPOG
            ``?o=`` workspace selector for the ``X-Databricks-Org-Id``
            routing header. Defaults to ``RUNNER_SERVER_URL`` so existing
            callers (which pass only the factory) need no change.
        """
    ...
    def auth_flow(
        ...
        """
        # SPOG workspace routing: on a unified-account host the request must
        # name the workspace or the proxy routes it to the account API proxy
        # (the forwarder's POST /events otherwise 403s). Empty for non-SPOG.
        # Set once here — it persists on the request across the retry yield.
        if self._server_url:
...
def create_app(
    ...
    server_client = httpx.AsyncClient(
        ...
        #
        # The SPOG workspace-routing header (empty unless this runner's host
        # is a unified-account workspace with a recorded ?o= selector) routes
        # these callbacks to the workspace instead of the account API proxy.
