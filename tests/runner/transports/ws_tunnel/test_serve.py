...
async def test_serve_tunnel_once_sends_bearer_header(
    ...
    # Non-SPOG server: no recorded ?o= selector, so no workspace-routing
    # header rides the handshake (keeps the asserted header set exact).
...
async def test_serve_tunnel_once_sends_spog_org_header(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A SPOG host's recorded ?o= selector rides the tunnel handshake.

    On a unified-account host the WS upgrade must name the workspace via
    ``X-Databricks-Org-Id`` or the proxy routes it to the account
    (shard-not-found). The selector is keyed by the server URL, not the ws
    tunnel URL, so the handshake resolves it from *server_url*.
