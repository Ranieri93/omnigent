...
def test_remote_headers_adds_spog_org_id_header(monkeypatch: pytest.MonkeyPatch) -> None:
    """A SPOG host's recorded ?o= selector rides every ad-hoc request.
    ...
    added here or the proxy routes them to the account API proxy. It
    accompanies whichever bearer the resolution chain produced.
    """
    monkeypatch.delenv("OMNIGENT_REMOTE_AUTH_TOKEN", raising=False)
    monkeypatch.setattr("omnigent.cli_auth.load_token", lambda _url: None)
...
def test_databricks_token_auth_sets_spog_org_header(monkeypatch: pytest.MonkeyPatch) -> None:
    """Every SDK-client request to a SPOG host carries the routing header.
